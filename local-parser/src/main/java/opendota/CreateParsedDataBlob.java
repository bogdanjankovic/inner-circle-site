...
class ParsedData {
    ...
    public Integer radiantScore = 0;
    public Integer direScore = 0;
...
class PlayerData {
    ...
    public Integer lastHits = 0;
    public Integer denies = 0;
    public Integer netWorth = 0;
    public Integer gpm = 0;
    public Integer xpm = 0;
    public Integer heroDamage = 0;
    public Integer towerDamage = 0;
    public Integer heroHealing = 0;
    public List<String> items = new ArrayList<>();
    public List<String> backpack = new ArrayList<>();
    public String neutral_item;
    public Boolean aghs_scepter = false;
    public Boolean aghs_shard = false;
    ...
    public List<Map<String, Object>> ability_upgrades = new ArrayList<>(); // Skill build tracking
    public List<Map<String, Object>> talents = new ArrayList<>(); // Talent tracking
}
...
public class CreateParsedDataBlob {
    ...
    public ParsedData createParsedDataBlob(List<Entry> entries) {
        ...
        finalizeSummaryStats(parsedData);

        return parsedData;
    }

    private void finalizeSummaryStats(ParsedData parsedData) {
        // Calculate duration from player times if not set
        if (parsedData.duration == null || parsedData.duration == 0) {
            int maxTime = 0;
            for (PlayerData p : parsedData.players) {
                if (p.times != null && !p.times.isEmpty()) {
                    int lastTime = p.times.get(p.times.size() - 1);
                    if (lastTime > maxTime) {
                        maxTime = lastTime;
                    }
                }
            }
            parsedData.duration = maxTime;
        }

        float minutes = parsedData.duration / 60.0f;
        if (minutes <= 0)
            minutes = 1;
        ...
        for (PlayerData p : parsedData.players) {
            ...
            for (Map.Entry<String, Integer> entry : p.damage.entrySet()) {
                String target = entry.getKey();
                int val = entry.getValue();
                if (target.contains("npc_dota_hero_") && !target.contains("illusion_")) {
                    heroDamage += val;
                } else if (target.contains("_tower") || target.contains("_rax") || target.contains("_fort")
                        || target.contains("_building")) {
                    towerDamage += val;
                }
            }

            for (Map.Entry<String, Integer> entry : p.healing.entrySet()) {
                heroHealing += entry.getValue();
            }

            p.heroDamage = heroDamage;
            p.towerDamage = towerDamage;
            p.heroHealing = heroHealing;

            // GPM & XPM - calculate total gold earned / total xp earned divided by minutes
            if (p.netWorth != null && p.netWorth > 0) {
                p.gpm = (int) (p.netWorth / minutes);
            } else if (p.gold_t != null && !p.gold_t.isEmpty()) {
                int lastGold = p.gold_t.get(p.gold_t.size() - 1);
                p.gpm = (int) (lastGold / minutes);
            }
            if (p.xp_t != null && !p.xp_t.isEmpty()) {
                int lastXp = p.xp_t.get(p.xp_t.size() - 1);
                p.xpm = (int) (lastXp / minutes);
            }
        }
    ...
    private void populate(Entry e, ParsedData container, Metadata meta) {
        switch (e.type) {
            case "match_id":
                container.matchId = e.longValue;
                break;
            case "duration":
                container.duration = e.value;
                break;
            case "winner":
                container.winner = e.key;
                break;
            case "radiant_score":
                container.radiantScore = e.value;
                break;
            case "dire_score":
                container.direScore = e.value;
                break;
            case "interval":
                if (e.slot != null && e.slot >= 0 && e.slot < container.players.size()) {
                    PlayerData p = container.players.get(e.slot);
                    if (e.kills != null)
                        p.kills = e.kills;
                    if (e.deaths != null)
                        p.deaths = e.deaths;
                    ...
                    if (e.hero_id != null)
                        p.heroId = e.hero_id;
                    if (e.longValue != null)
                        p.steamId = e.longValue;
                    if (e.unit != null) {
                        p.heroName = e.unit.replace("CDOTA_Unit_Hero_", "").replace("CDOTA_Hero_", "");
                    }
                    if (e.name != null)
                        p.name = e.name;
                    if (e.x != null && e.y != null) {
                        if (!p.lane_pos.containsKey(String.valueOf(e.time))) {
                            p.lane_pos.put(String.valueOf(e.time), new HashMap<>());
                        }
                        Map<String, Integer> tMap = p.lane_pos.get(String.valueOf(e.time));
                        tMap.put(String.valueOf(Math.round(e.x)), Math.round(e.y));
                    }
                    updatePlayerItems(p, e);
                }
                break;
            case "epilogue":
                if (e.slot != null && e.slot >= 0 && e.slot < container.players.size()) {
                    PlayerData p = container.players.get(e.slot);
                    // Force update inventory from epilogue if available
                    updatePlayerItems(p, e);
                }
                break;
            case "player_slot":
            ...
            case "CHAT_MESSAGE_SHARD_REWARD":
                if (e.player1 != null && e.player1 >= 0 && e.player1 < container.players.size()) {
                    if ("CHAT_MESSAGE_MINIBOSS_KILL".equals(e.type) || "CHAT_MESSAGE_SHARD_REWARD".equals(e.type)) {
                        container.players.get(e.player1).aghs_shard = true;
                    }
                }
            ...
            case "building_kill":
                ...
                break;
            case "draft_timings":
                container.draft_timings.add(deepCopy(e, Entry.class));
                break;
            ...
            case "ability_levels":
                ...
                String k = e.key;
                // STAGE 1: Determine the level. Priority: abilitylevel, then value, then level.
                Integer currentLevel = e.abilitylevel != null ? e.abilitylevel : (e.value != null ? e.value : e.level);
                if (currentLevel == null)
                    currentLevel = 0;

                Integer oldLevel = meta.ability_levels.get(e.unit).getOrDefault(k, 0);
                meta.ability_levels.get(e.unit).put(k, currentLevel);

                // STAGE 2: Filter for valid skill upgrades
                // 1. Must be after spawn/pre-game dump (typically -90 to -80 which has innates)
                // 2. Must be a level increase (oldLevel -> currentLevel)
                // 3. Must not be a "filler" or "innate" ability
                boolean isAfterSpawn = (e.time != null && e.time > -80);
                boolean isLevelIncrease = (currentLevel > oldLevel);

                boolean isSkilledAbility = k != null &&
                        !k.startsWith("seasonal_") &&
                        !k.startsWith("generic_") &&
                        !k.startsWith("plus_") &&
                        !k.startsWith("twin_gate") &&
                        !k.contains("_innate") &&
                        !k.contains("_intrinsic") &&
                        !k.startsWith("ability_") &&
                        !k.contains("abyssal_underlord_portal_warp");

                if (isAfterSpawn && isLevelIncrease && isSkilledAbility) {
                    Integer abilitySlot = meta.hero_to_slot.get(e.unit);
                    if (abilitySlot != null && abilitySlot >= 0 && abilitySlot < container.players.size()) {
                        PlayerData abilityPlayer = container.players.get(abilitySlot);

                        // Deduplicate simultaneous events in the same second for this player
                        boolean alreadySkilledInThisSecond = false;
                        if (!abilityPlayer.ability_upgrades.isEmpty()) {
                            Map<String, Object> lastU = abilityPlayer.ability_upgrades
                                    .get(abilityPlayer.ability_upgrades.size() - 1);
                            if (e.time != null && e.time.equals(lastU.get("time"))) {
                                alreadySkilledInThisSecond = true;
                            }
                        }

                        // RUBICK FIX: Filter out stolen spells from skill build
                        // Only allow abilities starting with "rubick_" or "special_bonus_"
                        // (talents/stats)
                        if ("npc_dota_hero_rubick".equals(abilityPlayer.heroName)) {
                            // Also need to handle "generic_hidden" or others if they sneak in, but
                            // mainly we want to exclude "disruptor_static_storm", "tidehunter_ravage", etc.
                            boolean isNativeAbility = k.startsWith("rubick_")
                                    || k.startsWith("special_bonus_")
                                    || k.equals("special_bonus_attributes");

                            if (!isNativeAbility) {
                                // It is a stolen spell (e.g. "faceless_void_chronosphere"), skip it for the
                                // skill build
                                break;
                            }
                        }

                        if (alreadySkilledInThisSecond) {
                            break;
                        }

                        Map<String, Object> upgrade = new HashMap<>();
                        upgrade.put("time", e.time);
                        upgrade.put("ability", k);

                        // Treat everything as a linear upgrade (talents, attributes, skills)
                        upgrade.put("level", abilityPlayer.ability_upgrades.size() + 1);
                        abilityPlayer.ability_upgrades.add(upgrade);

                        if (k.contains("bonus_unique")) {
                            // Also track in talents list just in case needed, but ability_upgrades is
                            // primary now
                            abilityPlayer.talents.add(upgrade);
                        }
                    }
                }
                break;
    ...
    private void handleArrayField(Entry e, PlayerData player) {
        if ("neutral_item_history".equals(e.type)) {
            ...
            if ("purchase_log".equals(e.type)) {
                if ("aghanims_shard".equals(e.key)) {
                    player.aghs_shard = true;
                }
                if ("ultimate_scepter".equals(e.key)) {
                    player.aghs_scepter = true;
                }
    ...
    private void handleMinibossKill(Entry e, List<Entry> output, Metadata meta) {
        ...
        minibossKill.player1 = e.player1; // Ensure player1 is preserved
    ...
    private void updatePlayerItems(PlayerData p, Entry e) {
        if (e.hero_inventory != null) {
            p.items.clear();
            p.backpack.clear();
            for (Item item : e.hero_inventory) {
                if (item != null && item.id != null) {
                    // Slots 0-5 are main inventory, 6-8 are backpack
                    // Ideally check item.slot
                    if (item.slot != null) {
                        if (item.slot >= 0 && item.slot <= 5) {
                            p.items.add(item.id);
                        } else if (item.slot >= 6 && item.slot <= 8) {
                            p.backpack.add(item.id);
                        }
                    } else {
                        // Fallback if slot is missing
                        p.items.add(item.id);
                    }
                }
            }
        }
        if (e.neutral_item != null) {
            p.neutral_item = e.neutral_item;
        }
        if (e.aghs_scepter != null && e.aghs_scepter) {
            p.aghs_scepter = true;
        }
        if (e.aghs_shard != null && e.aghs_shard) {
            p.aghs_shard = true;
        }
    }
    ...
    private List<Teamfight> processTeamfights(List<Entry> entries, Metadata meta) {
        ...
        teamfights = teamfights.stream().filter(tf -> tf.deaths >= 3).collect(Collectors.toList());

        for (

        Teamfight tf : teamfights) {
