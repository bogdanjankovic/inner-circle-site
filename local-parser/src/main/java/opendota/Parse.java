...
public class Parse {
    ...
    private boolean isMatchInfoProcessed = false;
    ...
    public Parse(InputStream input, OutputStream output, boolean blob) throws IOException {
        this(input, output, blob, false);
    }

    public Parse(InputStream input, OutputStream output, boolean blob, boolean websiteFormat) throws IOException {
        ...
        os = output;
        doBlob = blob;
        this.websiteFormat = websiteFormat;
        ...
        if (doBlob) {
            ...
            CreateParsedDataBlob creator = new CreateParsedDataBlob();
            ParsedData parsed = creator.createParsedDataBlob(this.finalList);
            if (websiteFormat) {
                // Output website-compatible JSON format
                WebsiteMatchData websiteData = WebsiteMatchData.fromParsedData(parsed, this.finalList);
                os.write(g.toJson(websiteData).getBytes());
            } else {
                // Output standard OpenDota format
                os.write(g.toJson(parsed).getBytes());
            }
        } else {
    ...
    public void output(Entry e) {
        if (!epilogue && gameStartTime == 0 && logBuffer != null) {
            ...
            finalList.add(e);
            // TODO if we don't want to buffer the entire log in memory and aren't
            // assembling blob we can write to output directly here
            // os.write((g.toJson(e) + "\n").getBytes());
        }
    }
    ...
    public void onAllChatMessage(Context ctx, CDOTAUserMsg_ChatMessage message) {
        ...
        System.err.println("Debug: Chat message detected: type=" + type + ", text=" + message.getMessageText());
        Entry entry = new Entry(time);
    ...
    public void onFileInfo(Context ctx, CDemoFileInfo message) {
        ...
        // Also emit final inventory states for all players
        Entity pr = ctx.getProcessor(Entities.class).getByDtName("CDOTA_PlayerResource");
        if (pr != null) {
            for (int i = 0; i < numPlayers; i++) {
                try {
                    // Use checks similar to onTickStart to find valid hero handles
                    Integer heroHandle = getEntityProperty(pr, "m_vecPlayerTeamData.%i.m_hSelectedHero",
                            validIndices[i]);
                    Entity eHero = ctx.getProcessor(Entities.class).getByHandle(heroHandle);

                    if (eHero != null) {
                        Entry epilogueEntry = new Entry();
                        epilogueEntry.type = "epilogue";
                        epilogueEntry.slot = i;
                        epilogueEntry.hero_inventory = getHeroInventory(ctx, eHero);
                        output(epilogueEntry);
                    }
                } catch (Exception e) {
                    // Ignore errors during epilogue dump
                }
            }
        }
        ...
        output(epilogueEntry);
        epilogue = true;
        // Some replays don't have a game start time and we never flush, so just do it
        // now
        flushLogBuffer();
    }
    ...
    public void onCombatLogEntry(Context ctx, CombatLogEntry cle) {
        try {
            ...
            // Process all combat log types that we've specifically handled
            // or just everything for now to be safe, filtering in CreateParsedDataBlob
    ...
    public void onEntityEntered(Context ctx, Entity e) {
        ...
        if (entityName.equals("CDOTAWearableItem")) {
            ...
            if ((neutralDropTeam != null && neutralDropTeam != 0)
                    && ((isNeutralActiveDrop != null && isNeutralActiveDrop)
                            || (isNeutralPassiveDrop != null && isNeutralPassiveDrop == true))) {
                Entry entry = new Entry(time);
        ...
        } else if (entityName.equals("CDOTA_NPC_Observer_Ward")) {
            Entry entry = new Entry(time);
            entry.type = "obs_log";
            entry.slot = getPlayerSlotFromEntity(ctx, e);
            if (entry.slot != null) {
                ...
                if (cx != null && cy != null) {
                    ...
                    entry.z = (cz != null && vz != null) ? getPreciseLocation(cz, vz) : 0f;
                    output(entry);
                }
            }
        } else if (entityName.equals("CDOTA_NPC_Sentry_Ward")) {
            Entry entry = new Entry(time);
            entry.type = "sen_log";
            entry.slot = getPlayerSlotFromEntity(ctx, e);
            if (entry.slot != null) {
    ...
    public void onTickStart(Context ctx, boolean synthetic) {
        ...
        if (grp != null && !isMatchInfoProcessed) {
            Number mid = getEntityProperty(grp, "m_pGameRules.m_unMatchID64", null);
            if (mid != null && mid.longValue() != 0) {
                Entry midEntry = new Entry(time);
                midEntry.type = "match_id";
                midEntry.longValue = mid.longValue();
                output(midEntry);
                isMatchInfoProcessed = true;
            }
        }

        if (grp != null) {
            Integer w = getEntityProperty(grp, "m_pGameRules.m_nGameWinner", null);
            if (w != null && (w == 2 || w == 3)) {
                if (winner != w) {
                    winner = w;
                    Entry winnerEntry = new Entry(time);
                    winnerEntry.type = "winner";
                    winnerEntry.key = (winner == 2 ? "Radiant" : "Dire");
                    output(winnerEntry);

                    Entry durationEntry = new Entry(time);
                    durationEntry.type = "duration";
                    durationEntry.value = time;
                    output(durationEntry);

                    // Emit team scores
                    Integer radiantScore = getEntityProperty(grp, "m_pGameRules.m_nRadiantTeamScore", null);
                    Integer direScore = getEntityProperty(grp, "m_pGameRules.m_nDireTeamScore", null);
                    if (radiantScore != null) {
                        Entry radiantScoreEntry = new Entry(time);
                        radiantScoreEntry.type = "radiant_score";
                        radiantScoreEntry.value = radiantScore;
                        output(radiantScoreEntry);
                    }
                    if (direScore != null) {
                        Entry direScoreEntry = new Entry(time);
                        direScoreEntry.type = "dire_score";
                        direScoreEntry.value = direScore;
                        output(direScoreEntry);
                    }
                }
            }
            ...
            if (draftStage == 2) {
                ...
                if (!isDraftStartTimeProcessed) {
                    Number iPlayerIDsInControl = getEntityProperty(grp, "m_pGameRules.m_iPlayerIDsInControl", null);
                    boolean isDraftStarted = iPlayerIDsInControl != null && iPlayerIDsInControl.longValue() != 0;
                    if (isDraftStarted) {
        ...
        if (pr != null) {
            ...
            if (!init) {
                ...
                while (added < numPlayers && i < 30) {
                    try {
                        ...
                        Number steamid = getEntityProperty(pr, "m_vecPlayerData.%i.m_iPlayerSteamID", i);
                        ...
                        if (playerTeam == 2 || playerTeam == 3) {
                            ...
                            steamid_to_playerslot.put(steamid != null ? steamid.longValue() : 0L, entry.value);
            ...
            if (init && !postGame && time >= nextInterval) {
                ...
                for (int i = 0; i < numPlayers; i++) {
                    ...
                    // facet/variant format and key location changed in 7.39, leaving this as a
                    // fallback
                    Integer variant = getEntityProperty(pr, "m_vecPlayerTeamData.%i.m_nSelectedHeroVariant",
                            validIndices[i]);
                    ...
                    entry.name = getEntityProperty(pr, "m_vecPlayerData.%i.m_iszPlayerName", validIndices[i]);
                    Number steamIdNum = getEntityProperty(pr, "m_vecPlayerData.%i.m_iPlayerSteamID", validIndices[i]);
                    entry.longValue = steamIdNum != null ? steamIdNum.longValue() : null;
                    ...
                    if (e != null) {
                        ...
                        // post-7.39 format for facets is a 128bit number, where last 32 bits represent
                        // the variant
                        // and the first 32 bits represent the hero id, which acts as the source for the
                        // facet
                        // (same as hero_id in all cases, except ability draft)
                        // 0xHHHH00000000VVVV
                        Number facet_key_num = getEntityProperty(e, "m_iHeroFacetKey", null);
                        if (facet_key_num != null) {
                            long facet_key = facet_key_num.longValue();
                        ...
                        if (hero > 0) {
                            ...
                            entry.neutral_item = getHeroNeutralItem(ctx, e);

                            // Check Scepter/Shard via modifiers
                            List<String> modifiers = getHeroModifiers(ctx, e);
                            boolean hasScepter = false;
                            boolean hasShard = false;

                            if (modifiers != null) {
                                for (String mod : modifiers) {
                                    if ("modifier_item_ultimate_scepter_consumed".equals(mod))
                                        hasScepter = true;
                                    if ("modifier_item_aghanims_shard".equals(mod))
                                        hasShard = true;
                                }
                            }

                            // Also check inventory for Scepter
                            if (!hasScepter && entry.hero_inventory != null) {
                                for (Item item : entry.hero_inventory) {
                                    if ("item_ultimate_scepter".equals(item.id)) {
                                        hasScepter = true;
                                        break;
                                    }
                                }
                            ...
                            entry.aghs_scepter = hasScepter;
                            entry.aghs_shard = hasShard;
            ...
            if (postGame && !isDotaPlusProcessed) {
                for (int i = 0; i < numPlayers; i++) {
                    ...
                    Number steamid = getEntityProperty(pr, "m_vecPlayerData.%i.m_iPlayerSteamID", i);
                    Long steamidLong = steamid != null ? steamid.longValue() : 0L;
                    if (steamid_to_playerslot.containsKey(steamidLong)) {
                        int playerslot = steamid_to_playerslot.get(steamidLong);
    ...
    private String getHeroNeutralItem(Context ctx, Entity eHero) {
        // Try slot 16 (Neutral Slot in recent patches)
        try {
            Item item = getHeroItem(ctx, eHero, 16);
            if (item != null) {
                // System.err.println("Found neutral item in slot 16: " + item.id);
                return item.id; // getHeroItem returns Item object with .id (name)
            }
            // Try slot 15 (sometimes swapped with TP?)
            Item item15 = getHeroItem(ctx, eHero, 15);
            if (item15 != null) {
                // System.err.println("Item in slot 15: " + item15.id);
                // Check if it's a neutral item? For now assume 16 is correct,
                // but if 16 is empty check 15 if logic requires.
                // Usually 15 is TP scroll, 16 is Neutral.
            }
    ...
    private List<String> getHeroModifiers(Context ctx, Entity eHero) {
        List<String> mods = new ArrayList<>();

        // Inventory Fallback for Scepter/Shard
        try {
            // Check inventory for Scepter/Shard items
            // Slots 0-5 (main), 16 (neutral), 6-8 (backpack - usually inactive but check
            // anyway?)
            // Active items are in 0-5.
            for (int i = 0; i <= 5; i++) {
                Item item = getHeroItem(ctx, eHero, i);
                if (item != null && item.id != null) {
                    if (item.id.equals("item_ultimate_scepter")) {
                        mods.add("modifier_item_ultimate_scepter"); // Synthetic modifier check
                    } else if (item.id.equals("item_aghanims_shard")) {
                        mods.add("modifier_item_aghanims_shard");
                    }
                }
            }
        } catch (Exception e) {
        ...
        try {
            Entities entities = ctx.getProcessor(Entities.class);
            StringTable stModifierNames = ctx.getProcessor(StringTables.class).forName("ModifierNames");
            int heroHandle = eHero.getHandle();

            // Real modifier scan
            Iterator<Entity> buffs = entities.getAllByDtName("CDOTA_Buff");
            while (buffs.hasNext()) {
                Entity buff = buffs.next();
                Integer parentHandle = buff.getProperty("m_hParent");
                if (parentHandle != null && parentHandle == heroHandle) {
                    Integer nameIdx = buff.getProperty("m_pModifier.m_nameStringableIndex");
                    if (nameIdx == null)
                        nameIdx = buff.getProperty("m_nameStringableIndex");

                    if (nameIdx != null && stModifierNames != null) {
                        String name = stModifierNames.getNameByIndex(nameIdx);
                        if (name != null)
                            mods.add(name);
                    }
                }
